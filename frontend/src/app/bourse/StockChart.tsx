...
const SHARED_LINE = { priceLineVisible: false, lastValueVisible: false, crosshairMarkerVisible: false } as const;
...
export default function StockChart({ data, indicators }: Props) {
  ...
  useEffect(() => {
    ...
    const chart = createChart(containerRef.current, {
      autoSize: true,
      layout: { background: { color: "#0d0d0d" }, textColor: "#6b7280" },
    ...
    chart.priceScale("right").applyOptions({ scaleMargins: { top: 0.02, bottom: mainBottom } });
    ...
    candles.setData(data.map((d) => ({ time: d.time as LWTime, open: d.open, high: d.high, low: d.low, close: d.close })));
    ...
    // EMAs
    for (const [key, period, color] of [["ema9", 9, "#f97316"], ["ema21", 21, "#a855f7"], ["ema50", 50, "#3b82f6"]] as [string, number, string][]) {
      if (!indicators.has(key)) continue;
      chart.addLineSeries({ ...SHARED_LINE, color, lineWidth: 1 }).setData(toLine(data, ema(closes, period)));
    }

    // SMAs
    for (const [key, period, color] of [["ma20", 20, "#fbbf24"], ["ma50", 50, "#10b981"], ["ma200", 200, "#6366f1"]] as [string, number, string][]) {
      if (!indicators.has(key)) continue;
      chart.addLineSeries({ ...SHARED_LINE, color, lineWidth: 1 }).setData(toLine(data, smaArr(closes, period)));
    }
    ...
    if (hasVol) {
      ...
      vol.setData(data.map((d) => ({ time: d.time as LWTime, value: d.volume ?? 0, color: d.close >= d.open ? "#16a34a50" : "#dc262650" })));
    }
    ...
    if (hasRsi) {
      ...
      const rsiS = chart.addLineSeries({ priceScaleId: "rsi", color: "#a78bfa", lineWidth: 1, priceLineVisible: false, lastValueVisible: true, crosshairMarkerVisible: false });
      chart.priceScale("rsi").applyOptions({ scaleMargins: { top: rsiTop, bottom: rsiBottom } });
      rsiS.setData(data.map((d, i) => i < 14 ? null : { time: d.time as LWTime, value: calcRsi(closes.slice(0, i + 1), 14) }).filter(Boolean) as { time: LWTime; value: number }[]);
    ...
    if (hasMacd) {
      ...
      chart.addLineSeries({ ...sharedM, color: "#3b82f6", lineWidth: 1 }).setData(toLine(data, line));
      chart.addLineSeries({ ...sharedM, color: "#f97316", lineWidth: 1 }).setData(toLine(data, signal));
      const histS = chart.addHistogramSeries({ ...sharedM });
      chart.priceScale("macd").applyOptions({ scaleMargins: { top: 0.71, bottom: macdBottom } });
      histS.setData(data.map((d, i) => isNaN(histogram[i]) ? null : { time: d.time as LWTime, value: histogram[i], color: histogram[i] >= 0 ? "#22c55e60" : "#ef444460" }).filter(Boolean) as { time: LWTime; value: number; color: string }[]);
    }
