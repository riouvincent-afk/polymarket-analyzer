...
export default function MarketCard({ market }: { market: Market }) {
  ...
  const categoryColor = CATEGORY_COLORS[market.category] ?? "text-gray-400 bg-gray-400/10";
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 flex flex-col gap-4 hover:border-gray-600 transition-colors">
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="text-xs text-gray-500">Ends {formatDate(market.endDate)}</span>
      </div>
      ...
      {/* Stats */}
      <div className="grid grid-cols-3 gap-2 pt-1 border-t border-gray-800 text-xs text-gray-400">
        <div>
        ...
        <div>
          ...
          <p className="text-white font-medium">{formatVolume(market.volumeTotal)}</p>
        </div>
        <div>
